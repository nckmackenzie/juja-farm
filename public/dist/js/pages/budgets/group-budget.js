...
async function checkYear() {
  if (!yearSelect.value || yearSelect.value == '') return;
  if (!groupSelect.value || groupSelect.value == '') return;
  ...
  const data = await sendHttpRequest(
    `${HOST_URL}/groupbudgets/checkyear?year=${yearVal}&group=${groupVal}&id=${id}`
  );
